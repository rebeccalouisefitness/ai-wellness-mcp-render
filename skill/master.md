...
- **HARD RULE — PHOTO MARKERS (semantic, capped at 3 per page).** Every personalized page (recruit landing, recruit thank-you, customer landing, customer thank-you) has EXACTLY 3 distributor photo slots, each semantically named: `photo_1` = hero image (top of page, looking at camera), `photo_2` = meet-me / split section ("Meet [Name]" portrait), `photo_3` = lifestyle / feature image (action shot or feature break). Templates MUST mark each personal slot with `data-pd="Photo" data-photo-slot="1|2|3"`. Any photo element WITHOUT `data-photo-slot` is a generic kit image — the loader MUST preserve it as-is and NEVER substitute it with a distributor photo. If a template has more than 3 `data-pd="Photo"` markers per page, the template is broken — abort and dispatch to Rebecca. Locked 2026-06-18 after Sinead's desktop view showed her personal photos in slots that should have been generic kit imagery.
- **HARD RULE — PRE-BAKED LOADER OUTPUT (strip marker UI on all 4 pages).** The per-distributor loader files (`the-{kit}-{slug}-loader.js` for any page: recruit landing, recruit thank-you, customer landing, customer thank-you) MUST ship with all marker UI stripped before publish: no HIDE/SHOW MARKERS toggle button anywhere in the loader's HTML or CSS, no visible `.aiw-legend` floating panel, all marker visual CSS (outline, box-shadow, `::before`/`::after` label tooltips, legend) omitted from the loader's `<style>` block. `data-pd` attributes may remain in the HTML for accessibility but produce NO visual styling on the live page. Markers exist ONLY in the master template for substitution — the per-distributor loader is a flat, clean, customer-facing build with no developer affordances. Applies equally to all 4 personalized pages. Locked 2026-06-18 after Simona's live recruit landing page shipped with the dev toggle button visible to public visitors.
